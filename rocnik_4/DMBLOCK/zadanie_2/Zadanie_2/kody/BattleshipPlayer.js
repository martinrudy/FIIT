...
let contractAddress = "0xb9b2A6a89ef46628645c6e6D17DF7b87B03d695a";
...
class BattleshipPlayer {
  ...
  */
  constructor(name, my_addr, opponent_addr, ante) {
    ...
    Battleship.events.OpponentLeft(function (err, result) {
      if (err) {
        return error(err);
      }
    
      console.log("Count was incremented by address: " + result);
    });
  }
    ...
    //    TODO make a call to the contract to store the bid
		...
		//					wei = eth*10**18
    // ##############################################################################
		Battleship.methods.store_bid().send({
      from: this.my_addr,
      value: ante*10**18
    });
	}
  ...
  */
  async initialize_board(initial_board) {
    ...
    // ##############################################################################
		Battleship.methods.store_board_commitment(commit).send({
      from: this.my_addr
    });
    return [commit, sig];
  }
  ...
  */
  ...
  async accuse_timeout() {
    ...
    // ##############################################################################
		Battleship.methods.claim_opponent_left(this.opp_addr).send({
      from: this.my_addr
    });
    ...
    // ##############################################################################
		Battleship.methods.handle_timeout(this.opp_addr).send({
      from: this.my_addr
    });
		console.log("handling timeout");

    return await Battleship.methods.is_game_over().call({from: this.my_addr})
  }
  ...
  */
    ...
    // ##############################################################################
    Battleship.methods.claim_timeout_winnings(this.opp_addr).send({
      from: this.my_addr
    });
    let gameover = await Battleship.methods.is_game_over().call({from: this.my_addr});
    if(gameover){
	...
	*/
    ...
    // ##############################################################################
		...
		if(typeof(this.last_guess) == 'undefined'){
      console.log("not guesses yet");
    }
    else{
      console.log(this.last_guess)
      Battleship.methods.accuse_cheating(this.last_guess['opening_nonce'], this.last_guess['proof'], this.last_guess['index'], this.opp_addr).send({
        from: this.my_addr});
    }
	...
	*/
	async forfeit_game() {
		...
		// ##############################################################################
		Battleship.methods.forfeit(this.opp_addr).send({
      from: this.my_addr
    });
    console.log("FF GG WP");
    return await Battleship.methods.is_game_over().call({ from: this.my_addr})
	}
}
